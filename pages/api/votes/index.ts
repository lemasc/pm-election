...
export async function exportVotes(req: NextApiSessionRequest): Promise<StudentExportList[]> {
  ...
  const login = await api.post(
    ...
    new URLSearchParams({
      ...
      syear: "2564",
      sclub: "",
      sclass: (req.body.class || req.query.class) as string,
      sroom: (req.body.room || req.query.room) as string,
    }),
    {
...
export default withAuth(
  withAPISession(async (req, res) => {
    ...
    if (!req.token?.admin) return res.status(403).json({ success: false });
    try {
      if (!req.body || !checkParameters(req.body, ["class", "room"]))
        return res.status(400).json({ success: false });
      const votes = await exportVotes(req);
      return res.status(200).json(votes.map((v) => Object.values(v)));
    } catch (err) {
      console.error(err);
